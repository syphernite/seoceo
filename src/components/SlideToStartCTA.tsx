...
/**
 * Tactile "slide to start" CTA.
 * Mobile fix: use touch-action: pan-y on the track so horizontal drags go to us, not page scroll.
...
export default function SlideToStartCTA({ to, label = "Slide to start", className = "" }: Props) {
  ...
  const startDrag = (e: React.PointerEvent<HTMLDivElement>) => {
    if (done) return
    setDragging(true)
    // Capture pointer on the TRACK, not the child node.
    ;(e.currentTarget as Element).setPointerCapture(e.pointerId)
    setProgress(posToProgress(e.clientX))
    // Prevent native gestures initiating scroll on some browsers.
    e.preventDefault()
  }
  ...
  return (
    <div
      ...
      className={[
        ...
        "cursor-pointer",
        // Tailwind touch-action helpers. Prevent horizontal page pan, allow vertical scroll.
        "touch-pan-y",
        // Reduce page overscroll bounce on iOS within this control.
        "overscroll-contain",
        className
      ].join(" ")}
      // Fallback if Tailwind touch utilities are unavailable.
    ...
    >
      ...
      {/* knob — keep events on track by not capturing here */}
      <div
        className={[
        ...
        style={{
          ...
          // Ensure pointer stays on track element for consistent capture.
          pointerEvents: "none"
        }}
      >
